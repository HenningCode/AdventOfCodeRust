...
impl PokerHand {
    ...
    fn get_card_value(c: char) -> u32 {
        let card_values = HashMap::from([('T', 10), ('J', 1), ('Q', 12), ('K', 13), ('A', 14)]);
    ...
    fn determine_type(&mut self) {
        let mut num_chars = self.hand.chars().fold(HashMap::new(), |mut acc, c| {
            *acc.entry(c).or_insert(0) += 1;
            acc
        });
        if num_chars.contains_key(&'J') && num_chars.len() != 1 {
            let val_j = num_chars[&'J'];
            num_chars.remove_entry(&'J');
            if let Some((key_max, value_max)) = num_chars.iter().max_by_key(|x| x.1) {
                num_chars.insert(*key_max, val_j + value_max);
            }
        }
...
QQQJA 483
JJJJJ 555";
        assert_eq!(parse_input(input), 5905);
    }
    ...
    fn get_card_value_test() {
        ...
        assert_eq!(PokerHand::get_card_value('J'), 1);
